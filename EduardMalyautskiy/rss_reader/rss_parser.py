...
from converters import convert_to_html, save_to_html, save_to_pdf
from os import path
...
def parse_cli_args(commands=None):
    ...
    parser.add_argument('--date', help='It should take a date in %Y%m%d format. For example: --date 20191020. '
    ...
    parser.add_argument('--to_html', nargs='?', default=None,
                        help='Upload result to html-file in folder "output-files", file name is current date with ms.')
    parser.add_argument('--to_pdf', nargs='?', default=None,
                        help='Upload result to pdf-file in folder "output-files", file name is current date with ms.')
...
class ReadRss:
    ...
    @staticmethod
    def get_news_from_db(date, url=None, limit=None, db='rss_cache.db'):
        """
        ...
        with DataConn(db) as conn:
            ...
            for item in result:
                ...
                article["Media"] = path.join(path.abspath(path.dirname(__file__)), 'images', str(item[0]) + '.jpg')
                article["GUID"] = item[8]
        ...
        with DataConn() as conn:
            for article in feed.articles_dicts:
                ...
                if entry is None:
                    ...
                    conn.execute(
                        'insert into news(Feed, Title, Link, Date, Description, Source, Media, GUID, Url) '
                        'values (?, ?, ?, ?, ?, ?, ?, ?, ?)',
                        [article.get('Feed'), article.get('Title'), article.get('Link'), article.get('Date'),
                         ...
                         remove_quots(namespace.source)])
                    with open(path.join(path.abspath(path.dirname(__file__)), 'images', str(max_id + 1) + '.jpg'),
                              "wb") as f:
                        f.write(requests.get(article.get('Media')).content)
            for res in conn.execute('select count(*) from news'):
                print('В базе сохранено всего ' + str(res[0]) + ' новостей')
    ...
    if namespace.to_pdf:
        ...
        try:
            if path.isabs(namespace.to_pdf) and path.isdir(namespace.to_pdf):
                save_to_pdf(feed.articles_dicts, namespace.to_pdf)
            else:
                raise IncorrectPath
        except IncorrectPath:
            print_and_exit('Incorrect path to save file.')
